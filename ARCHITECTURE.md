...
When initialized, the SDK checks `window.trikomi_config` to authenticate the host application and unlock premium modules. To prevent stale browser caching when SDK binaries are updated, all entrypoints import `SDK_VERSION` and append dynamic version query strings (`?v=${SDK_VERSION}`).
